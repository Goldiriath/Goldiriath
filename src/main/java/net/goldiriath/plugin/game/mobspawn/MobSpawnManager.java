    ...
    public void onSignDelete(BlockBreakEvent event) {
        ...
        for (MobSpawn loopSpawn : spawns) {
            ...
            if (!loopSpawn.getLocation().equals(event.getBlock().getLocation())) {
                plugin.logger.info(Locations.format(loopSpawn.getLocation()) + " != " + Locations.format(event.getBlock().getLocation()));
                continue;
            }
        ...
        if (spawn == null) {
            plugin.logger.warning("Could not delete mob spawn at location " + Locations.format(event.getBlock().getLocation()) + ". Could not find assocated spawner!");
